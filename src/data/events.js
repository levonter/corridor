...
export function eventToGeoJSON(ev){if(!ev)return{type:'FeatureCollection',features:[]};const f=[];(ev.incidents||[]).forEach(i=>i?.a!=null&&i?.o!=null&&f.push({type:'Feature',geometry:{type:'Point',coordinates:[i.o,i.a]},properties:{name:i.ti||'',severity:i.s||'medium',type:i.tp||'displacement',date:i.dt||'',description:i.d||'',actor:i.ac||'',org:i.og||''}}));(ev.corridor||[]).forEach(p=>p?.a!=null&&p?.o!=null&&f.push({type:'Feature',geometry:{type:'Point',coordinates:[p.o,p.a]},properties:{name:p.n||'',type:'waypoint'}}));if(ev.corridor?.length>1)f.push({type:'Feature',geometry:{type:'LineString',coordinates:ev.corridor.filter(p=>p?.a!=null&&p?.o!=null).map(p=>[p.o,p.a])},properties:{name:(ev.name||'Event')+' Corridor',type:'corridor'}});return{type:'FeatureCollection',properties:{event:ev.name||'',exported:new Date().toISOString()},features:f}}
export function eventToCSV(ev){if(!ev)return'';const r=[['Type','Name','Lat','Lon','Severity','Date','Description','Actor','Org']];(ev.incidents||[]).forEach(i=>i?.a!=null&&r.push(['Incident',`"${i.ti||''}"`,i.a,i.o,i.s||'',i.dt||'',`"${(i.d||'').replace(/"/g,'""')}"`,i.ac||'',i.og||'']));(ev.corridor||[]).forEach(p=>p?.a!=null&&r.push(['Waypoint',p.n||'',p.a,p.o,'','',`"${p.d||''}"`,'-','-']));return r.map(x=>x.join(',')).join('\n')}
export function eventToReport(ev){if(!ev)return'';const l=[];l.push(`# ${ev.name||'Event'}\n**${(ev.severity||'medium').toUpperCase()}** | ${ev.status||'active'} | Updated: ${ev.updatedAt||'N/A'}\n`);l.push(`## Brief\n${ev.brief||'N/A'}\n`);if((ev.briefs||[]).length){l.push('## Briefs Timeline');ev.briefs.forEach(b=>b&&l.push(`- [${b.ts?new Date(b.ts).toLocaleDateString():'N/A'}] ${b.text||''}${b.archived?' *(archived)*':''}`));l.push('')}if((ev.incidents||[]).length){l.push(`## Incidents (${ev.incidents.length})`);ev.incidents.forEach(i=>i&&l.push(`- **${i.ti||'Incident'}** (${i.dt||'N/A'}) — ${(i.s||'medium').toUpperCase()} — ${i.d||''}`));l.push('')}if((ev.notebook||[]).length){l.push('## Field Notes');ev.notebook.forEach(n=>n&&l.push(`- [${n.author||'Unknown'}] ${n.text||''}`));l.push('')}l.push(`---\n*Corridor Planner v4 — ${new Date().toISOString().slice(0,10)}*`);return l.join('\n')}
